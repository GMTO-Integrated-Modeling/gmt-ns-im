...
async fn calibrate(channel: usize, path: impl AsRef<Path>) -> anyhow::Result<()> {
    ...
    let m1_rbm = Signals::from((m1_rbm, 6000));
    ...
    actorscript!(
        ...
        5: om24[Sh24Frame]! -> k24[SensorData]
            -> recon[M2FSMFsmCommand] -> int[M2FSMFsmCommand]! -> {fsms}
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut mount_el = Arrow::from_parquet("sh48_1murd_m1-es-global-tip.parquet")?;
    ...
    let mut mount_az = Arrow::from_parquet("sh48_1murd_m1-es-global-tilt.parquet")?;
    ...
    println!(
        "Saving SH48 to M1 assembly reconstructor to {:?}",
        path.canonicalize()?
    );
